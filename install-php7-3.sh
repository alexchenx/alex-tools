...
cd php-7.3.19
./configure --prefix=${php_home} --enable-fpm --enable-mysqlnd --enable-mbstring --enable-pcntl --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-zlib --with-openssl --with-gd --with-curl=/data/app/curl
make && make install
if [ $? -ne 0 ]; then
