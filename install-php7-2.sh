...
cd php-7.2.31
./configure --prefix=${php_home} --enable-fpm --enable-mysqlnd --enable-pcntl --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-zlib --with-openssl --with-gd --with-curl=/data/app/curl
make && make install
if [ $? -ne 0 ]; then
...
"install-php.sh" 81L, 2473C written                                                                                                                                                                                    
[root@qem.qooco.com tmp]# cat 
[root@qem.qooco.com tmp]# cat install-php.sh 
#!/bin/bash
